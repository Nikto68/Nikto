...
final class TelegramEntityUtils
{
    ...
    /** Stands in for whichever custom emoji an id "looks like" — unknowable from the id alone. */
    private const EMOJI_PLACEHOLDER_FALLBACK = '⭐';

    /**
     * Recognizes [digits] anywhere in $text (e.g. "سلام [6258234230396949553]")
     * and turns each one into a real custom_emoji entity, exactly as if the
     * operator had sent that premium emoji themselves and the bot had read
     * its entity back. This is what lets an id copied from an emoji-pack
     * channel be dropped straight into any template or button-icon field as
     * plain bracketed text, with no Telegram Premium account of the bot
     * owner's own needed to author it.
     *
     * Every id gets the same fixed fallback character (there is no way to
     * know, from the id alone, which standard emoji it visually resembles)
     * — same idea as the admin-typed-emoji flow, just without requiring a
     * real premium emoji to be sent first. Pre-existing entities (bold,
     * quote, an already-set custom_emoji, ...) have their offsets shifted
     * exactly like renderTemplate() shifts them for a {placeholder}.
     *
     * @param array<int,array<string,mixed>> $entities
     * @return array{text:string, entities:array<int,array<string,mixed>>}
     */
    public static function applyEmojiPlaceholders(string $text, array $entities): array
    {
        if (!preg_match_all('/\[(\d{5,20})\]/', $text, $matches, PREG_OFFSET_CAPTURE)) {
            return ['text' => $text, 'entities' => $entities];
        }

        $fallback = self::EMOJI_PLACEHOLDER_FALLBACK;
        $fallbackLen = self::utf16Length($fallback);
        $tokens = [];
        foreach ($matches[0] as $idx => $full) {
            $byteStart = $full[1];
            $utf16Start = self::utf16Length(substr($text, 0, $byteStart));
            $utf16TokenLen = self::utf16Length($full[0]);
            $tokens[] = [
                'byte_start' => $byteStart,
                'byte_len' => strlen($full[0]),
                ...
                'utf16_end' => $utf16Start + $utf16TokenLen,
                'value' => $fallback,
                'delta' => $fallbackLen - $utf16TokenLen,
                'emoji_id' => $matches[1][$idx][0],
            ];
        }
        ...
        foreach ($tokens as $token) {
            $workingEntities[] = [
                'type' => 'custom_emoji',
                'offset' => self::mapOffset($tokens, $token['utf16_start'], false),
                'length' => $fallbackLen,
                'custom_emoji_id' => $token['emoji_id'],
            ];
        }

        $newText = $text;
        foreach (array_reverse($tokens) as $token) {
            $newText = substr_replace($newText, $fallback, $token['byte_start'], $token['byte_len']);
        }

        return ['text' => $newText, 'entities' => $workingEntities];
    }
