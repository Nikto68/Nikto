...
function pxDefaults() {
    return [
        ...
        // ایموجی پریمیوم — با دریافت ایموجی در ربات کدشان را می‌گیرید
        'emoji' => [
...
function pxFetch($fresh = false) {
    ...
    if (!$fresh) {
        ...
        // قیمت (خریدِ شماره، مینی‌اپ، پنل — نه فقط پاسخِ قیمتِ چت که
...
/**
 * ⚡ رندِ عدد، دقیقا هم‌سطحِ چیزی که رویِ کارت/کپشن نشان داده می‌شود —
 * فقط برایِ کلیدِ کش، نه برایِ خودِ نمایش.
 *
 * قبلا کلیدِ کارت/فایل‌آیدیِ تلگرام از رویِ قیمتِ خام (با اعشارهایی که
 * اصلا نمایش داده نمی‌شدند) ساخته می‌شد. تومان که همیشه گرد است ولی
 * لحظه‌به‌لحظه به‌اندازه‌ی کسری تکان می‌خورد، یعنی کلید تقریبا هرباری که
 * قیمت تازه می‌شد عوض می‌شد — حتی وقتی خودِ عددِ نمایشی دقیقا همان بود.
 * نتیجه: هر ~۱ دقیقه (تازه‌شدنِ کشِ قیمت)، اولین کسی که هر نمادی را
 * می‌پرسید، کارت را از صفر می‌ساخت و از نو آپلود می‌کرد — رندر ارزان
 * است (~۷۰ میلی‌ثانیه) ولی آپلودِ عکس به تلگرام کند است؛ همین یعنی
 * تقریبا هر درخواست، کند. حالا کلید با همان رندی که رویِ کارت می‌رود
 * ساخته می‌شود، پس تا وقتی رقمِ نمایشی واقعا عوض نشده، همان عکسِ قبلی
 * (با همان file_id) دوباره استفاده می‌شود — نه رندرِ دوباره، نه آپلودِ
 * دوباره.
 */
    ...
    if ($png !== null || $cacheKey !== '') {
        ...
        if (function_exists('adminAlertOnce')) {
            adminAlertOnce('px_photo', "⚠️ <b>کارت قیمت فرستاده نشد</b>\n\n" .
                ...
                "پنل ← 💹 قیمت لحظه‌ای ← 🖼 کارت را خاموش کنید.");
        }
    }
...
function pxHandleText($text, $chatId, $replyTo = null) {
    ...
    if ($ak !== null) {
        ...
        $ck  = 'a|' . $ak . '|' . pxKeyRound($price, $a['unit']) . '|' . round($chg, 2) . '|' . botUsername();
    ...
    $ck  = 'c|' . $sym . '|' . round($usd * $irtRate) . '|' . round($chg, 2) . '|' . botUsername();
    $needsRender = (maCacheGet(pxPhotoIdKey($ck), 86400) ?? '') === '';
    if ($needsRender) pxWaitReact($chatId, $replyTo, true);
    $png = $needsRender
...
function pxAdminList($chatId, $msgId, $kind) {
    ...
    $t = $title . "\n\n";
    if ($kind === 'pxe') $t .= "کد ایموجی پریمیوم را با دریافت ایموجی می‌گیرید.\n\n";
