...
function maDefaultTg() {
    ...
    return $d = [
        ...
        'ui' => [
            ...
            'buy_now'    => 'خرید',
            'topup_btn'  => '＋ شارژ',
            // 🛡 چند خطِ اعتماد — همان چیزی که مینی‌اپ شماره مجازی بالای «درباره» دارد، ولی یکی کافی نبود
            'trust_ttl'  => 'با اطمینان خرید کنید',
            'trust_txt'  => 'سفارش‌ها خودکار پردازش می‌شوند؛ اگر مشکلی پیش بیاید مبلغ به کیف پول شما برمی‌گردد.',
            'trust_ttl2' => 'پرداخت امن',
            'trust_txt2' => 'کیف‌پولِ داخلِ ربات؛ اطلاعاتِ بانکی هیچ‌جا ذخیره نمی‌شود.',
            'trust_ttl3' => 'پشتیبانیِ ۲۴ ساعته',
            'trust_txt3' => 'برای هر سوال یا مشکلی، همیشه یک نفر پاسخ‌گوست.',
        ],
...
function maDefaultNum() {
    ...
    return $d = [
        ...
        'ui' => [
            ...
            // 📖 بلوکِ معرفی در صفحه‌ی خانه. «ویژگی‌ها» با | جدا می‌شوند
            //    و هر تکه «ایموجی فاصله متن» است.
            'about_ttl' => 'شماره مجازی چیست؟',
            'about_txt' => 'شماره مجازی به شما این امکان را می‌دهد که با حفظ اطلاعات شخصی خود، '
                         . 'در دنیای دیجیتال فعالیت داشته باشید. ساخت حساب کاربری در شبکه‌های '
                         . 'اجتماعی و سایت‌های خارجی با شماره مجازی امکان‌پذیر است. برای ارتباطات '
                         . 'شخصی و تجاری خود می‌توانید از همین‌جا و از کشورهای مختلف با قیمت‌های '
                         . 'متنوع شماره مجازی بگیرید.',
            'feats'     => '💸 قیمت ارزان | ⭐ کیفیت بالا | ⚡ تحویل فوری | '
                         . '💳 پرداخت آنلاین | 🛡 امنیت بالا | 🎧 پشتیبانی آنلاین',
            'trust_ttl' => 'با اطمینان خرید کنید',
            'trust_txt' => 'اگر کد نرسد، مبلغ خودکار به کیف پول شما برمی‌گردد.',
...
/**
 * 💎 تون/ترون به استپرِ کم/زیاد ارتقا پیدا کنند.
 *
 * maGet() برای فیلدهای تازه با پیش‌فرض ادغام می‌شود، ولی «ask» یک
 * فیلدِ تازه نبود — نصب‌های قدیمی از قبل مقدارش را (همان 'qty' ی
 * بسته‌های آماده) ذخیره کرده بودند، پس ادغام همیشه همان مقدارِ کهنه
 * را نگه می‌داشت و استپرِ جدید هیچ‌وقت روشن نمی‌شد، حتی روی نصب‌هایی
 * که کدشان کاملا به‌روز است. فقط همین دو آیتم و فقط همین فیلد.
 */
function maFixCoinAsk() {
    foreach (['i_ton', 'i_trx'] as $coinId) {
        maSet('tg', function (&$a) use ($coinId) {
            foreach ($a['items'] ?? [] as $i => $it) {
                if ((string)($it['id'] ?? '') === $coinId && (string)($it['ask'] ?? '') !== 'qty_wallet')
                    $a['items'][$i]['ask'] = 'qty_wallet';
            }
        });
    }
}
...
/**
 * $fresh=true یعنی از دیسک بخوان، نه از حافظه‌ی درون‌درخواستیِ load().
 *
 * چرا لازم است: اگر همین پردازه قبل‌تر همین فایل را (حتی یک‌بار) خوانده
 * باشد، load() دیگر دوباره از دیسک نمی‌خواند — برای اکثرِ جاها همین
 * درست است، ولی وقتی پردازه پشتِ یک قفل (مثلِ ضدِ ازدحامِ pxFetch)
 * منتظر مانده تا پردازه‌ی دیگری کش را تازه کند، باید بعدِ گرفتنِ قفل
 * واقعا دوباره از دیسک بخواند، وگرنه همان عکسِ کهنه‌ی قبل از قفل را
 * می‌بیند و انگار هیچ‌کس چیزی تازه نکرده.
 */
function maCacheGet($key, $ttl, $fresh = false) {
    $c = load('ma_cache', $fresh);
    $x = $c[$key] ?? null;
...
/**
 * 🧹 پاک‌سازیِ دوره‌ایِ ma_cache.
 *
 * این فایل یک JSONِ یک‌تکه است — هر خواندن/نوشتن، کلِ فایل را
 * می‌خواند/می‌نویسد. کلیدهایی که یک قابلیتِ حذف‌شده گذاشته (و دیگر
 * هیچ‌جا خوانده نمی‌شوند) تا ابد همان‌جا می‌مانند و فقط حجمش را
 * بالا می‌برند — دقیقاً همان چیزی که در «⚡️ سرعت ربات» هشدارش
 * دیده می‌شود. اینجا هم کلیدهای شناخته‌شده‌ی مرده را صریح پاک
 * می‌کند، هم هر ورودیِ خیلی کهنه را — فارغ از اینکه کلیدش چیست.
 */
function maCachePrune() {
    // 🎁 اجاره‌ی گیفت حذف شد؛ gr_list دیگر هیچ‌جا خوانده نمی‌شود.
    $deadKeys = ['gr_list'];
    $maxAge   = 259200; // ۳ روز — خیلی بیشتر از TTLِ واقعیِ هر کدام از این کش‌ها
    $now      = time();
    mutate('ma_cache', function (&$c) use ($deadKeys, $maxAge, $now) {
        foreach ($deadKeys as $k) unset($c[$k]);
        foreach ($c as $k => $x) {
...
/**
 * 🔖 نسخه‌ی صفحه‌ی مینی‌اپ.
 *
 * ⚠️ این همان چیزی است که نبودنش باعث می‌شد «تغییرات ظاهری اصلا
 *    انجام نشده» به نظر برسد.
 *
 *    سرآیندِ no-store روی خودِ پاسخ هست، ولی وب‌ویوِ تلگرام روی
 *    اندروید صفحه‌ی مینی‌اپ را جدا نگه می‌دارد و برای همان آدرس،
 *    نسخه‌ی قبلی را باز می‌کند — گاهی روزها. پس آدرس باید عوض شود،
 *    نه فقط پاسخ.
 *
 *    نسخه را از زمانِ تغییرِ خودِ فایل‌های نما می‌گیریم: هر بار که
 *    فایلِ تازه‌ای آپلود کنید، آدرس هم عوض می‌شود و وب‌ویو مجبور است
 *    از نو بخواند. آپلود نکنید، آدرس هم ثابت می‌ماند و کش کار خودش
 *    را می‌کند.
 */
function maViewVer() {
    static $v = null;
...
/**
 * 🗄 انبارِ سفارش‌های مینی‌اپ — روی SQLite، نه یک فایلِ JSON.
 *
 * قبلا هر تغییرِ یک سفارش (پرداخت، تحویل، رد...) کلِ فایلِ ma_orders را
 * می‌خواند، دوباره می‌ساخت و زیرِ یک قفلِ سراسری می‌نوشت — یعنی هر سفارش،
 * فارغ از این‌که کدام یکی تغییر کرده، منتظرِ همان یک قفل می‌ماند؛ و
 * MaOrder::get() هم برای پیدا کردنِ یک سفارش، کلِ فایل را decode
 * می‌کرد. با بایگانی (maOrdersArchive) فایلِ داغ کوچک نگه داشته
 * می‌شد، ولی مشکلِ اصلی — «هر عملیات = کل جدول» — سرِ جایش بود.
 *
 * حالا مثلِ کاربرها و بازی‌ها: هر سفارش یک ردیفِ مستقل، با ستون‌های
 * ایندکس‌شده برای user_id/status — یعنی «سفارش‌های من» و شمارشِ
 * وضعیت‌ها دیگر کلِ جدول را به PHP نمی‌آورند، خودِ SQLite با ایندکس
 * جواب می‌دهد.
 */
...
/** یک‌بار، فقط وقتی ma_orders.sqlite تازه ساخته می‌شود: هرچه در فایل‌های JSON قدیمی بود کوچ می‌کند */
...
class MaOrder
{
    ...
    /** همه‌ی سفارش‌های داغ — فقط برای فهرست/آمار در پنل. در مسیرِ داغِ هر تیک استفاده نشود. */
    ...
    /** جدولِ داغ، و اگر نبود بایگانی — تا سفارش قدیمی هم پیدا شود. هر دو با یک SELECT روی id، نه خواندنِ کلِ جدول */
    ...
    public static function create($app, $uid, $uname, $item, $qty, $total, $field) {
        ...
        if ($db) {
            ...
            $stmt->bindValue(':app', (string)$app, SQLITE3_TEXT);
    ...
    /**
     * تغییر یک سفارش، اتمیک، فقط همان یک ردیف — نه کلِ جدول (مثلِ mutateUser/gmSetGame).
     * اگر کال‌بک چیزی برگرداند، همان برمی‌گردد؛ چیزی که $fn روی آرایه عوض
     * کرده، فارغ از مقدارِ برگشتی‌اش، ذخیره می‌شود — درست مثلِ رفتارِ قبلیِ
     * mutate() که همیشه می‌نوشت، حتی اگر $fn نهایتا false برگرداند.
     */
    public static function set($id, callable $fn) {
        ...
        try {
            ...
            $up->bindValue(':app', (string)($o['app'] ?? ''), SQLITE3_TEXT);
     ...
     *
     * قبلا این یعنی کلِ جدول به PHP می‌آمد، فیلتر و مرتب می‌شد — حالا
     * خودِ SQLite با ایندکسِ (user_id, created_at) جواب می‌دهد.
     */
    public static function forUser($uid, $limit = 10, $app = null) {
        $db = maOrdersDb();
        if (!$db) return [];
        $sql = 'SELECT data FROM orders WHERE user_id = :uid';
        if ($app !== null) $sql .= ' AND app = :app';
        $sql .= ' ORDER BY created_at DESC LIMIT :lim';
        $stmt = $db->prepare($sql);
        $stmt->bindValue(':uid', (int)$uid, SQLITE3_INTEGER);
        if ($app !== null) $stmt->bindValue(':app', (string)$app, SQLITE3_TEXT);
...
function maAutoFulfill($orderId, $manual = false) {
    ...
    // 🔒 اگر قبلا رویِ پنل ثبت شده، دیگه هیچ‌وقت دوباره اجرا نشو — حتی
    // با دکمه‌ی «تلاش دوباره». اجرایِ دوباره یعنی یک سفارشِ دیگر رویِ
    // پنل و یک بار دیگر پرداختِ واقعیِ TON. اگر فقط قدمِ امضایِ ولت
    // شکست خورده بود، باید دستی رسیدگی شود (یا با دکمه‌ی «✅ بسته شد»)،
    // نه با اجرایِ دوباره‌ی کلِ زنجیره.
    if (!empty($o['placed'])) {
        return [false, 'این سفارش قبلا رویِ پنل ثبت شده — برایِ جلوگیری از پرداختِ دوباره، دوباره اجرا نمی‌شود. اگر تراکنشِ ولت ناقص مانده، دستی رسیدگی کنید.'];
    }
    ...
    if (!function_exists('axQuietAuto') || !axQuietAuto())
        notifyAdmins(
...
function maOrdersArchive($days = 0, $limit = 4000) {
    ...
    // هرچه هنوز در جریان است اصلا از SQLite بیرون نمی‌آید
...
/** هر کلید دترمینیستیک به یکی از N تکه می‌رود — پخشِ قفل‌ها روی چند فایلِ کوچک، نه یک فایلِ بزرگِ مشترک */
 ...
 */
/**
 * ⚡ هر دو تابعِ زیر (نانس و ضدِ تکرار) قبلا یک فایلِ مشترک برای همه‌ی
 * کاربرها بودند — دقیقا همان مشکلی که maRateFile برایش شارد ساخته شد:
 * با کاربرهای هم‌زمانِ زیاد، همه‌ی ثبتِ سفارش‌های کلِ مینی‌اپ پشتِ یک
 * قفلِ سراسری صف می‌کشیدند، برای دو چکِ کاملا مستقل از هم. حالا مثلِ
 * محدودیتِ نرخ، شارد شده‌اند.
 */
...
function maBootUnified() {
    ...
    foreach (maKeys() as $k) {
        ...
        // 📁 num (ده‌ها کشور) و react (چندتا زیرسرویسِ ری‌اکشن/استوری) هرکدام
        // چند دسته‌ی داخلی دارند که در فروشگاهِ یکپارچه فقط شلوغیِ ردیفِ
        // دسته‌بندی را کنارِ دسته‌های واقعیِ tg زیاد می‌کند — این‌جا (فقط در
        // نمایِ یکپارچه؛ کاتالوگِ خودِ اپ دست‌نخورده می‌ماند) هرکدام زیرِ
        // یک دسته‌ی واحد به‌نامِ خودِ اپ جمع می‌شوند. اسمِ زیرسرویس/کشور
        // همچنان توی اسمِ خودِ آیتم هست، پس چیزی گم نمی‌شود.
        if ($k === 'num' || $k === 'react') {
            $collN = 0;
            $collId = '__' . $k . '_all';
            foreach ($appItems as &$it) { $it['cat'] = $collId; $collN++; }
            unset($it);
            if ($collN > 0) {
                $cats[] = ['id' => $collId, 'name' => (string)$a['title'],
                           'emoji' => trim((string)($a['btn']['emoji'] ?? '')) ?: '💠', 'n' => $collN, 'app' => $k];
            }
        } else {
...
function maApi() {
    ...
    if ($action === 'adm_get' || $action === 'adm_set') {
        ...
        if (!isAdmin($uid)) maApiOut(['ok' => false, 'error' => 'not_found'], 404);
    ...
    if ($action === 'order') {
        ...
        if (!isAdmin($uid) && function_exists('masterJoinMissing')) {
...
function maMarkPaid($id, $payMethod) {
    ...
    // 📡 گزارش خرید — فقط از یک جا: 📡 کانال‌های متصل (تنظیمِ گزارش پرداختِ
    // جداگانه‌ی این بخش عمدا حذف شد، چون همان چیز را دوباره می‌گفت)
...
function maPalettes() {
    return [
        ...
        'emerald' => ['name' => '⚫️ مشکی و سبز',     'c1' => '#0B7A3E', 'c2' => '#39D98A', 'c3' => '#F4F6F2', 'bg' => '#050B07'],
    ];
}
