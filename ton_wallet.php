...
/**
 * ⚠️ وقتی ارسالِ تراکنش شکست می‌خورد، شبکه‌ی تون همیشه معلوم نمی‌کند
 * که واقعا رد شده یا رفته ولی جوابش گم شده (مثلا LITE_SERVER_UNKNOWN).
 * قبل از این‌که به ادمین بگوییم «نشد» — که یعنی دکمه‌ی «تلاش دوباره»
 * را می‌زند و اگر واقعا رفته بود پول دوبار می‌رود — seqnoِ ولت را
 * بعد از شکست دوباره می‌پرسیم: اگر جلو رفته، یعنی چیزی روی همین ولت
 * نشسته (به‌احتمال زیاد همین تراکنش) و باید «نامعلوم» بگوییم، نه «نشد».
 *
 * برگشت: پیامِ هشدارِ «نامعلوم» اگر seqno جلو رفته، وگرنه null (یعنی
 * واقعا رد شده، برگشتِ پول و «تلاش دوباره» امن است).
 */
function tonAmbiguousSendMessage($seqnoBefore, $seqnoAfter, $sendErr, $address) {
    if ($seqnoAfter === null || $seqnoAfter <= (int)$seqnoBefore) return null;
    return '⚠️ نامعلوم — ممکن است همین تراکنش قبول شده باشد (seqnoِ ولت از ' .
        (int)$seqnoBefore . ' به ' . $seqnoAfter . ' رفته)، ولی جوابِ شبکه گم شد. ' .
        'قبل از هر «تلاش دوباره»، حتما آدرسِ ولت (' . mb_substr((string)$address, 0, 16) . '…) را ' .
        'در tonviewer.com بررسی کنید تا پول دوبار نرود.' .
        "\n\nخطای اصلیِ شبکه: " . mb_substr((string)$sendErr, 0, 150);
}
