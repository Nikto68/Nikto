 ...
 * همان اطلاعات را یک‌جا با فرم می‌گیرد و به maOrderSubmit() می‌سپارد —
 * هیچ منطقِ قیمت/پرداخت اینجا تکرار نشده.
 *
 * ظاهر: تمام‌صفحه، شیشه‌ای، فقط سیاه‌وسفید (به‌جایِ رنگ، وضعیت با
 * آیکون/وزنِ فونت/شفافیت نشان داده می‌شود) — طبق خواستِ صریحِ کارفرما،
 * جدا از پوسته‌ی رنگیِ «فروشگاهِ یکپارچه».
 */
...
img{max-width:100%}
.wrap{position:relative;z-index:1;max-width:560px;margin:0 auto;padding:14px 15px calc(128px + var(--safe))}

.top{position:sticky;top:0;z-index:9;display:flex;align-items:center;gap:10px;
  margin:0 0 14px;padding:12px 14px;border-radius:18px;
  border:1px solid var(--line);background:rgba(10,10,10,.72);
  backdrop-filter:blur(var(--blur));-webkit-backdrop-filter:blur(var(--blur))}
.top h1{flex:1;margin:0;font-size:15px;font-weight:800}
...
.spcard .e{font-size:20px;margin-bottom:5px}
...
.btn{flex:1;border:1px solid var(--line);background:var(--pane2);color:var(--ink);
  border-radius:13px;padding:12px;font-size:12.5px;font-weight:800;font-family:inherit;cursor:pointer}
...
<div class="foot" id="foot" style="display:none">
  <div class="footin">
    ...
    <button class="submit" id="submitBtn">ثبتِ نهایی</button>
  </div>
</div>
...
var S = { qty: F.min || 1, speed: (F.speeds && F.speeds[0]) ? F.speeds[0].id : '', link: '', pay: B.wallet_ok ? 'wallet' : 'manual', adminOk: false, busy:false };
...
function curTotal(){
  ...
  var per = F.per || 1000;
  var rate = curRate();
  return Math.round((rate * (S.qty / per)) * 100) / 100;
}
...
if (P.closed) {
  ...
  html += '<div class="card"><p style="margin:0;font-size:13px;color:var(--dim);text-align:center">' + esc(msg) + '</p></div>';
...
api('order_boot', {}, function(j){
  $('balSpan').textContent = fa(j.balance || 0);
  if (j.closed) {
    var msg = j.closed === 'bought' ? 'این محصول را قبلا خریده‌اید.'
            : j.closed === 'full'   ? 'ظرفیتِ این محصول تکمیل شده است.'
            : 'این محصول در حال حاضر در دسترس نیست.';
    $('formArea').innerHTML = '<div class="card"><p style="margin:0;font-size:13px;color:var(--dim);text-align:center">' + esc(msg) + '</p></div>';
...
$('submitBtn').onclick = function(){
  if (S.busy) return;
  if (F.on && F.ask_link && !S.link) { showErr('لینک کانال را وارد کنید.'); return; }
  if (F.on && F.ask_admin && !S.adminOk) { showErr('اول باید ربات را ادمینِ کانال کنید و «بررسیِ دوباره» بزنید.'); return; }
  S.busy = true;
  ...
  api('order_submit', { link: S.link, qty: S.qty, speed_id: S.speed, pay: S.pay }, function(j){
    ...
    $('submitBtn').textContent = 'ثبتِ نهایی';
    if (j.need_admin) { showErr(j.message || 'ابتدا ربات را ادمینِ کانال کنید.'); return; }
