 ...
 * 👑 چند مدیر.
 *
 * قبلا فقط ADMIN_ID اجازه داشت — یعنی دقیقا یک نفر. حالا هرکسی که در
 * ADMIN_IDS باشد کامل به پنل دسترسی دارد و همه‌ی گزارش‌ها (رسید، خرید،
 * خطای فنی…) را می‌گیرد. ADMIN_ID همچنان اولی از همین فهرست است — فقط
 * برای جاهایی که یک «صندوقِ وضعیتِ گفتگو»ی مشترک لازم دارند.
 */
function isAdmin($uid) {
    return in_array((int)$uid, array_map('intval', ADMIN_IDS), true);
}

/** یک پیام را به همه‌ی مدیرها می‌فرستد — نه فقط اولی */
...
/**
 * 📋 فهرستِ سرویس‌های پنل — کش‌شده، تا ادمین به‌جای حدس‌زدنِ شماره‌ی
 * سرویس، از یک لیستِ واقعی انتخاب کند.
 *
 * چرا کش: این پاسخ می‌تواند صدها ردیف باشد؛ اگر هر بار که صفحه‌ی
 * محصول باز می‌شود دوباره از پنل گرفته شود، هم کند است هم بی‌خودی
 * رویِ حسابِ پنل فشار می‌آورد. یک ساعت برای لیستی که به‌ندرت عوض
 * می‌شود کافی است؛ دکمه‌ی «🔄 بروزرسانی» همیشه هست برای وقتی واقعا لازم شد.
 */
function smmServicesCached() {
    $c = maCacheGet('smm_services', 3600);
    return is_array($c) ? $c : [];
}
    ...
    if (isset($update['callback_query'])) {
        ...
        if ($data === 'adm_web' || $data === 'adm_sup' || $data === 'adm_prods'
            ...
            editMsg(BOT_TOKEN, $chatId, $msgId,
                ...
                "🎯 سفارش ممبر · 🤝 ربات‌های شریک · 👥 کاربران\n" .
...
function runBackgroundQueues() {
    ...
    // 🧹 پاک‌سازیِ کلیدهای مرده/کهنه‌ی ma_cache — هر ۶ ساعت کافی است.
    //    (مثلاً gr_list که بعدِ حذفِ اجاره‌ی گیفت دیگر هیچ‌جا خوانده
    //    نمی‌شود ولی تا پاک نشود، تو فایل می‌ماند و فقط حجم اضافه می‌کند)
